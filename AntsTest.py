...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        nearplace = self.place
        count = 0
        while nearplace != beehive:
            if count < self.min_range:
                nearplace = nearplace.entrance
                count += 1
                continue
            elif nearplace.bees != []:
                return rANTdom_else_none(nearplace.bees)
            nearplace = nearplace.entrance
            count += 1
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.place.bees==[]:
            ...
            if self.digesting==0:
                self.now_bee = self.place.bees[0]
                self.eat_bee(self.now_bee)
                self.now_bee.armor = 0
                print("Ok")
            elif self.digesting!=0 and self.digesting != self.time_to_digest:
                self.eat_bee(self.now_bee)
            if self.digesting == self.time_to_digest:
                self.digesting = 0
                Bee.reduce_armor(self.now_bee, 0)
        # END Problem 6
# BEGIN Problem 7
...
hungry = HungryAnt()            # A Bee with 1000 armor
